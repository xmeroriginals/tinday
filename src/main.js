const CL_SERVER_IP = "wss://cl.mikedev101.cc/";
      ...
      const INSTA_PROFILE_URL = "https://instagram.com/xmerofficial";
      ...
      const replyPreviewContent = document.getElementById(
        ...
        if (file.type.startsWith("image/") && file.size > 1024 * 1024) {
          try {
            displaySystemNotification(
              "Büyük resim işleniyor, lütfen bekleyin..."
            );
            fileDataUrl = await compressAndEncodeImage(file);
          } catch (error) {
            displaySystemNotification(
              "Resim işlenirken bir hata oluştu.",
              "error"
            );
            return;
          }
      ...
      function startReply(messageId) {
        ...
        strongEl.textContent = truncateText(
          replyingToMessage.originalSender,
          20
        );
      ...
      async function processMessageContent(content) {
        ...
        processedText = processedText.replace(urlRegex, (url) => {
          try {
            ...
            if (
              hostname === "giphy.com" &&
              urlObj.pathname.startsWith("/gifs/")
            ) {
      ...
      const displayMessage = async (data, isSentByMe, isOfficial = false) => {
        ...
        if (data.reply) {
          ...
          replyContent.textContent = truncateText(
            data.reply.originalContent,
            100
          );
      ...
      function fadeAndStop(audioElement, duration = 300) {
        return new Promise((resolve) => {
          ...
          const fadeOutInterval = setInterval(() => {
            const newVolume =
              audioElement.volume - startVolume / (duration / 50);
            if (newVolume > 0) {
      ...
      function initializePeerEvents(p) {
        ...
        p.on("error", (err) => {
          if (err.type === "peer-unavailable") {
            ...
            displaySystemNotification(`Arama Sorunu`, "error");
          }
        });
      ...
      declineCallBtn.addEventListener("click", () => {
        const isRejecting =
          !callPanel.classList.contains("in-call") && currentCall;
