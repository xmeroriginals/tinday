...
document.addEventListener("DOMContentLoaded", async function () {
  ...
  try {
    ...
    document.getElementById("snap-footer").style.display = "none";
    return;
  }

  const snapContainer = document.getElementById("snap-container");
  const snapFooter = document.getElementById("snap-footer");
  ...
  snapData.forEach((snapItem) => {
    ...
    if (snapItem.verified === true) {
      ...
      badgeElement.innerHTML = verifiedBage;
  ...
  window.addEventListener("resize", debounce(manageLayout, 250));
});
